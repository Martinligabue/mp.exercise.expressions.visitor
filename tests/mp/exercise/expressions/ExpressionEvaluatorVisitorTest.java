...
public class ExpressionEvaluatorVisitorTest {
  ...
  public void testEvalConstant() {
    ...
    assertEquals(
      true,
      new Constant(true).accept(new ExpressionEvaluatorVisitor())
    );
    assertEquals(
      "Hello",
      new Constant("Hello").accept(new ExpressionEvaluatorVisitor())
    );
  }
  ...
  public void testEvalEqualsTwoConstants() {
    assertEquals(
      true,
      new Equal(new Constant(10), new Constant(10))
        .accept(new ExpressionEvaluatorVisitor())
    );
    assertEquals(
      false,
      new Equal(new Constant(1), new Constant(10))
        .accept(new ExpressionEvaluatorVisitor())
    );
    assertEquals(
      false,
      new Equal(new Constant("hello"), new Constant(10))
        .accept(new ExpressionEvaluatorVisitor())
    );
  }
