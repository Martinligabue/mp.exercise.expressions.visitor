...
public class ExpressionTypeSystemVisitor
  implements ExpressionVisitor<Class<?>> {

  private List<Class<?>> expectedTypes = Arrays.asList(
    Integer.class,
    String.class,
    Boolean.class
  );
  ...
  public Class<?> visitConstant(Constant c) {
    ...
    if (!expectedTypes.contains(type)) throw new TypeSystemException(
  ...
  private void checkIntegerType(Class<?> type) {
    if (type != Integer.class) throw new TypeSystemException(
      "Expected Integer but was " + type.getSimpleName()
    );
  }
  ...
  public Class<?> visitEqual(Equal e) {
    ...
    if (leftType != rightType) throw new TypeSystemException(
      String.format(
